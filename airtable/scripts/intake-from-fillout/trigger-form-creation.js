const table = base.getTable('tblsnJnJ4ubpZFLwq'); // All Contacts Master Table
// When run from a button field, the script uses the button's record.
const record = await input.recordAsync('Choose a record', table);
if (!record) {
    console.error("No record selected!");
} else {
    await triggerLinkUpdate(table, record);
}
